...
public actor BackendTerminalCompatibilitySession {
    ...
    ///
    /// The lease is renewed before each frame. An input failure is terminal for
    /// this dedicated lane, so it cannot perturb the process-wide canonical
    /// frontend connection or create an ambiguous retry.
