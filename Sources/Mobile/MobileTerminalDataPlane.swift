...
actor PersistentMobileTerminalDataPlane: MobileTerminalDataPlane {
    ...
    func replay(surfaceID: UUID) async throws -> MobileTerminalDataPlaneReplay {
        ...
        await storePending(surfaceID: surfaceID, attachment: attachment)
    ...
    private func storePending(
        ...
        while pendingOrder.count >= maximumPendingReplayCount,
            ...
            await evicted.attachment.session.close()
        }
        let id = UUID()
        ...
        producer = Task {
            do {
                ...
                if let requestedCursor {
                    ...
                    transportBase = max(snapshot.sequence, UInt64(snapshot.replay.count))
                }
